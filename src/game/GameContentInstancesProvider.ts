...
export default class GameContentInstancesProvider<TContentInstance>
  implements ContentInstancesProvider<TContentInstance>
{
  ...
  async get(contentName: string): Promise<TContentInstance[]> {
    ...
    const contentInstances = game.objects
      .filter((x) => x.contentName == contentName)
      .map((x) => x as TContentInstance);
